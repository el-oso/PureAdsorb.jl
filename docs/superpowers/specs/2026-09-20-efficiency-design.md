...
- ☐ **Rejection rule (exact).** `widom` computes `W = exp(−ΔU/kT)` in the float type `F`; `W` is exactly `0.0` when `ΔU/kT > θ_F`, with `θ_F` the smallest value for which `exp(−F(θ_F)) == 0` (tested, not hard-coded from memory; about 745.2 in Float64 and 104 in Float32). An insertion is rejected only if a rigorous lower bound on `ΔU` exceeds `(θ_F + 2)·kT + 1e-5·B_s` (the last term covers the pair term's approximation error and floating-point summation error, both proportional to the magnitude of the summed terms); its `W` and `ΔU·W` are then recorded as exactly zero, so `μ_ex`, `K_H`, `q_st` and their errors are those of the single-phase calculation.
- ☐ **Lower bound.** `ΔU = Σ_{a,h} u_ah(r_ah) + U_recip + c_s`, with `u_ah(r) = LJ_ah(r)·[r < r_lj] + KE q_a q_h · pair_erfc(αr)/r·[r < r_ew]`. For a trigger pair at distance `r`: `u_ah(r) ≥ LJ_ah(r) − |K_ah|/r` when `K_ah = KE q_a q_h < 0`, else `≥ LJ_ah(r)`. For every other pair: when `K_ah ≥ 0`, `u_ah ≥ −ε_ah`; when `K_ah < 0`, let `r0_ah` be the zero of `u_ah` below its minimum (bisection on the host), then `u_ah ≥ 0` on `(0, r0]` and `u_ah ≥ −ε_ah − |K_ah|·erfc(α r0)/r0` on `[r0, r_c]`. `B_s` is the sum of those magnitudes over all sites and all host atoms of system `s`, plus the reciprocal bound `R_s = KE Σ_k kprefactor_k · 2|S_host,k| · Σ_a|q_a|`. `B_s` is computed at batch construction, stored per system, and is `Inf` (no rejection in that system) if any pair has `ε_ah = 0` with `K_ah < 0`.
- ☐ **Core radii.** Per `widom` call (they depend on `T`), per system, per (guest site `a`, host LJ type `t`): `ρ_at` is the first root, scanning up from `r → 0`, of `LJ_at(r) − |K_min(a,t)|/r = (θ_F + 2)·kT + B_s − c_s`, where `K_min(a,t)` is the most negative `K_ah` over the system's atoms of type `t` (zero if none is negative). Every `r < ρ_at` satisfies the rejection condition. Stored as `ρ²`, in a table indexed by a batch-local compact type index.
- ☐ **Compact types.** `FrameworkBatch` remaps LJ types to the types actually present in the batch (framework atoms and guest sites); `types`, `sigma`, `epsilon` use the compact index. The mapping is kept for error messages.
- ☐ **Phase 0 kernel.** One work-item per insertion. For each site, the 27-cell (reach from `max ρ`, computed per system per call) neighborhood of the site's own home cell is scanned with one minimum image per atom; the flag is set if any `r² < ρ²_at`. Output: one `UInt8` per insertion. Same GPU-safety rules as the energy kernel.
- ☐ **Compaction and phase 1.** The host reads the chunk's flags, builds the survivor index list in order (runs of one framework stay adjacent), uploads it, and launches the energy kernel over `nsurvivors` work-items, each reading its pose through the index. Rejected insertions add to the sample counts with zero weight. A chunk with no survivors launches nothing. A device-side scan replaces the host pass only if the flag copy is measured to matter.
- ☐ **Full energy path.** The energy kernel loops linearly over the system's atoms with one minimum image per atom (the fastest form in the E2 width table for this system size); the cell arrays serve phase 0. `cellwidth` default chosen by measurement among 2, 3, 4 Å for phase 0 cost and bytes per framework.
- ☐ **Zero-weight accumulation.** The host accumulates `ΔU·W` as zero whenever `W == 0`, also on the single-phase path: in Float32 a site within about 10⁻³ Å of a host atom overflows the LJ term to `Inf`, and `Inf · 0` is `NaN`. Tested with a pose placed on a host atom.
- ☐ **Gate before building kernels:** measure on the CPU, for CO2 in RUBTAK 3×3×3 at 298.15 K, the rejection fraction the rigorous `ρ_at` achieves in both precisions, next to the fraction of exact zeros (68% / 78%). Expected from the bound's size (`B_s` of order 10⁵ kT, `ρ ≈ 0.37σ ≈ 1.1 Å`): about 40%. If it is below 25%, stop and report instead of building.
- ☐ **Tests.** `θ_F` by construction; `B_s` against a brute-force evaluation of the bounded quantity on random poses (the bound holds for every sampled pose: `ΔU − u_trigger ≥ −B_s + c_s`); no rejected pose has `W > 0` when its full energy is computed (≥ 10⁵ poses, both precisions); two-phase `widom` results equal single-phase results exactly (`==`) for fixed seeds on the CPU backend, for one framework, several frameworks, a neutral guest, a framework where `B_s = Inf`, and chunks with zero survivors and with all survivors; GPU results within the existing ulp-level tolerance; kUPS cross-code item unchanged; audit covers both kernels' callees.
- ☐ Measured and recorded: rejection fraction, phase 0 time, end-to-end `widom` time and kernel-path time, RTX 3050 and R9700, both precisions.
