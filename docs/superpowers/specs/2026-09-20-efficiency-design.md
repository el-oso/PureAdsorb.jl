...
- ☑ **Rejection rule (exact).** `widom` computes `W = exp(−ΔU/kT)` in the float type `F`; `W` is exactly `0.0` when `ΔU/kT > θ_F`, with `θ_F` the smallest value for which `exp(−F(θ_F)) == 0` (tested, not hard-coded from memory: 745.1332191019412 in Float64, 103.972084 in Float32). An insertion is rejected only if a rigorous lower bound on `ΔU` exceeds `(θ_F + 2)·kT − ` (see the **Core radii** deviation below) `+ B_s`; its `W` and `ΔU·W` are then recorded as exactly zero, so `μ_ex`, `K_H`, `q_st` and their errors are those of the single-phase calculation (verified `==` in the two-phase/single-phase test).
...
- ☑ **Core radii.** Per `widom` call (they depend on `T`), per system, per (guest site `a`, host LJ type `t`): `ρ_at` is the first root, scanning up from `r → 0`, of `LJ_at(r) − |K_min(a,t)|/r = (θ_F + 2)·kT + B_s − c_s` (`find_rho2`), where `K_min(a,t)` is the most negative `K_ah` over the system's atoms of type `t` (zero if none is negative, `kmin_table`). Every `r < ρ_at` satisfies the rejection condition. Stored as `ρ²`, in a table indexed by a batch-local compact type index. **Deviation from this literal equation:** the "Rejection rule" bullet's margin is `(θ_F+2)·kT + 1e-5·B_s`, but this equation (also taken verbatim from the spec) omits the `1e-5·B_s` term — as written, `ρ_at` guarantees only `ΔU_ideal > (θ_F+2)·kT`, not the `+1e-5·B_s` margin the rejection rule states is needed to absorb `pair_erfc_dev`'s approximation error and floating-point summation error. Implemented exactly as the spec states in both places rather than reconciling them; the "no rejected pose has `W > 0`" test (2×10⁵ poses total, both precisions, on the real system) found zero counterexamples, so the gap did not manifest measurably here, but it is a real inconsistency between the two bullets as written, not a rounding note.
...
- ☑ **Gate before building kernels:** measured on the CPU (JuliaMCP session), for CO2 in RUBTAK 3×3×3 at 298.15 K, 40,000 poses: rejection fraction 41.34% (Float64) / 41.37% (Float32), next to the fraction of exact zeros 68.58% / 78.19% (matching the design's recorded 68%/78% baseline). `B_s` = 127,118 kT in both precisions (order 10⁵ kT as expected); `ρ_at` in [0.9185, 1.2016] Å (Float64) / [0.9189, 1.2021] Å (Float32) (`ρ ≈ 0.3–0.4σ`, as expected). Both rejection fractions are above the 25% stop threshold and close to the ~40% estimate — continued to the kernels.
