...
- ☑ **Rejection rule (exact).** `widom` computes `W = exp(−ΔU/kT)` in the float type `F`; `W` is exactly `0.0` when `ΔU/kT > θ_F`, with `θ_F` the smallest value for which `exp(−F(θ_F)) == 0` (tested, not hard-coded from memory: 745.1332191019412 in Float64, 103.972084 in Float32). An insertion is rejected only if a rigorous lower bound on `ΔU` exceeds `(θ_F + 2)·kT + safety + B_s − c_s`, with the safety term derived in "Core radii" below; its `W` and `ΔU·W` are then recorded as exactly zero, so `μ_ex`, `K_H`, `q_st` and their errors are those of the single-phase calculation (verified `==` in the two-phase/single-phase test, including a reproducer where the Lennard-Jones cutoff is shorter than the core radii — see "Core radii" below).
...
- ☑ **Core radii.** Per `widom` call (they depend on `T`), per system, per (guest site `a`, host LJ type `t`): `ρ_at` is the first root, scanning up from `r → 0`, of `LJ_at(r) − |K_min(a,t)|/r = (θ_F + 2)·kT + safety + B_s − c_s` (`find_rho2`/`build_rejection_tables`), clamped to `r_lj` for the same reason as `r0_ah` above, where `K_min(a,t)` is the most negative `K_ah` over the system's atoms of type `t` (zero if none is negative, `kmin_table`). Every `r < ρ_at` satisfies the rejection condition. Stored as `ρ²`, in a table indexed by a batch-local compact type index. The safety term is `safety = 2·n·eps(F)·(B_s + |c_s|) + 4e-6·B_s`, with `n = N_sites·natoms_s + nk_s + 8`: `2·n·eps(F)·(B_s+|c_s|)` bounds recursive-summation rounding error for `n` terms of magnitude at most `B_s + |c_s|` (the standard `(n-1)·u·Σ|x_i|` bound with unit roundoff `u = eps(F)/2`); `4e-6·B_s` covers `pair_erfc_dev`'s own relative error (measured up to 1.51e-6 in Float32 over `[0, PAIR_ERFC_XMAX]`), applied once per Coulomb term. `find_rho2` and `find_r0` bisect to a relative interval width of `4·eps(T)` (not a fixed iteration count), with an iteration cap that throws rather than looping past it; `find_rho2` requires `margin > 0` (throws by name otherwise) and returns `ρ² = 0` directly for a zero `epsilon_at`, both checked before any loop. `build_rejection_tables` runs its per-system loop under `Threads.@threads`, one memo `Dict` per thread keyed on `(σ, ε, kmin_at, margin)` (a batch of identical tiled systems shares the same margin and per-type values, so only the first system per thread pays a fresh bisection).
...
- ☑ Measured and recorded (RTX 3050, `bench/gpu`, RUBTAK 3×3×3 + CO2, commit 6ef0c7d): rejection fraction (kernel path) 40.5% both precisions; phase 0 / phase 1 / total kernel-path time for a 65,536-insertion chunk, cellwidth 2 Å: Float64 7.26 ms / 838.5 ms / 845.7 ms (against 3.7 / 836.9 / 840.6 ms at 3653c6f); Float32 1.18 ms / 38.4 ms / 39.5 ms (against 0.68 / 38.7 / 39.4 ms at 3653c6f); end-to-end `widom` time for 10⁶ insertions, one framework: Float64 13.04 s (76,715 ins/s, against 13.02 s at 3653c6f); Float32 0.672 s (1.489M ins/s, against 0.674 s at 3653c6f). Phase 0 costs more per site now that each site scans its own stencil (about 2× at Float64, 1.7× at Float32), but stays a small fraction of the kernel path, so the total and end-to-end times are unchanged within measurement noise. Speedup against the E2b baseline (commit 082c656: whole kernel 63.4 ms Float32, 1245.9 ms Float64): Float32 1.60×, Float64 1.47×. R9700 measurement stays open (galen not touched).
  ...
  Cellwidth sweep for phase 0 (RTX 3050, 65,536-insertion chunk, `bench/gpu/cellwidth_sweep.jl`, mirroring `bench/widom_bench.jl`'s kernel-only path; measured before this fix wave's per-site stencil change, at commit da327a6):
