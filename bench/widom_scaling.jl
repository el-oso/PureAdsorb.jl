...
for nsys in nsys_grid
    ...
    println("nsys=$nsys chunk=$chunk batch=$(round(nsys * bytes_per_system / 2^30; digits = 2)) GiB median=$(median(times)) s ips=$(chunk / median(times))")
    flush(stdout)
    dbatch = nothing
    GC.gc(true)
end
...
meta = (;
    ...
    bytes_per_system, nsys_grid, min_chunk,
)
...
tag = length(nsys_grid) == 1 ? "_nsys$(only(nsys_grid))" : ""
