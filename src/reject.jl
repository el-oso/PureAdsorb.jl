...
"""
function find_r0(σ::T, ε::T, K::T, alpha::T) where {T}
...
"""
    hardcore_bound(guest, sigma, epsilon, positions, types, charges, atoms, alpha, kprefactor, Shost) -> B_s
...
`pair_energy` can take anywhere on `(0, cutoff]` (`-ε` when the Coulomb term `K = KE·q_a·q_h` is
non-negative; `-ε - |K|·pair_erfc_dev(alpha·r0)/r0` at `r0 = find_r0(σ, ε, K, alpha)` when `K`
is negative, since the Coulomb term's magnitude is largest, for `r ≥ r0`, at `r0` itself), plus
the reciprocal-space cross-term bound `KE·Σ|q_guest|·Σ_k 2·kprefactor_k·|Shost_k|` (from
`|Re(conj(Shost)·Sg)| ≤ |Shost|·|Sg| ≤ |Shost|·Σ|q_guest|`). Returns `Inf` when any pair combines
an attractive Coulomb term with no Lennard-Jones repulsion at all (ε zero, K negative): the pair
energy is then unbounded below as `r → 0` and no finite bound exists.
"""
function hardcore_bound(
        guest::Guest{T, N}, sigma, epsilon, positions, types, charges, atoms, alpha::T, kprefactor, Shost
    ) where {T, N}
    acc = zero(T)
    for a in 1:N
        gt = guest.types[a]
...
"""
    find_rho2(sigma_at, epsilon_at, kmin_at, margin) -> ρ²
...
"""
function find_rho2(sigma_at::T, epsilon_at::T, kmin_at::T, margin::T) where {T}
