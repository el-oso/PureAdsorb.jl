...
@testsnippet RejectFixture begin
    ...
    function min_pair_energy(g, ff, b, pos, gsites, α, atoms, A, invA)
        u_trigger = Inf
        for a in eachindex(g.sites), h in atoms
            Δ = PureAdsorb.minimum_image(A, invA, pos - b.positions[h]) + gsites[a]
            r = norm(Δ)
            r < ff.cutoff || continue
            σ = ff.sigma[g.types[a], b.types[h]]
            ε = ff.epsilon[g.types[a], b.types[h]]
    ...
    function count_falsepositives(g, ff, b, rho2, atoms, α, kT, ::Type{T}; nposes) where {T}
...
@testitem "hardcore_bound holds against a random-pose brute-force check" setup = [RejectFixture] begin
    ...
    Bs = PureAdsorb.hardcore_bound(g, ff.sigma, ff.epsilon, b.positions, b.types, b.charges, atoms, α, b.kprefactor, b.Shost)
    @test isfinite(Bs)
    @test Bs > 0
    ...
    for _ in 1:2000
        ...
        u_trigger = min_pair_energy(g, ff, b, pos, gsites, α, atoms, A, invA)
...
@testitem "find_rho2 rejection never has a nonzero Boltzmann weight" setup = [RejectFixture] begin
    for T in (Float64, Float32)
        ...
        ntypes = maximum(view(b2.types, atoms))
        Bs = PureAdsorb.hardcore_bound(g2, ff2.sigma, ff2.epsilon, b2.positions, b2.types, b2.charges, atoms, α, b2.kprefactor, b2.Shost)
        @test isfinite(Bs)
        kmin = PureAdsorb.kmin_table(g2, b2.types, b2.charges, atoms, ntypes)
        ...
        n_rejected, all_zero = count_falsepositives(g2, ff2, b2, rho2, atoms, α, kT, T; nposes = 100_000)
        @test all_zero
        @test n_rejected > 0
    end
end
