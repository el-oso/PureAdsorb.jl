...
- ☑ **Rejection rule (exact).** `widom` accumulates `W = exp(−ΔU/kT)` on the host in Float64 regardless of `FrameworkBatch`'s own float type `F` (a well only about 88.7 kT deep already overflows `exp` in Float32); `W` is exactly `0.0` when `ΔU/kT > θ`, with `θ = theta_F(Float64)` the smallest value for which `exp(−θ) == 0` in Float64 (tested, not hard-coded from memory: 745.1332191019412). An insertion is rejected only if a rigorous lower bound on `ΔU` exceeds `(θ + 2)·kT + safety + B_s − c_s`, with the safety term derived in "Core radii" below; its `W` and `ΔU·W` are then recorded as exactly zero, so `μ_ex`, `K_H`, `q_st` and their errors are those of the single-phase calculation (verified `==` in the two-phase/single-phase test, for both `F`, including a reproducer where the Lennard-Jones cutoff is shorter than the core radii — see "Core radii" below).
- ☑ **Lower bound.** `ΔU = Σ_{a,h} u_ah(r_ah) + U_recip + c_s`, with `u_ah(r) = LJ_ah(r)·[r < r_lj] + KE q_a q_h · pair_erfc(αr)/r·[r < r_ew]` (`r_lj`, `r_ew` the Lennard-Jones and Ewald cutoffs). For a trigger pair at distance `r`: `u_ah(r) ≥ LJ_ah(r) − |K_ah|/r` when `K_ah = KE q_a q_h < 0`, else `≥ LJ_ah(r)`. For every other pair: when `K_ah ≥ 0`, `u_ah ≥ −ε_ah`; when `K_ah < 0`, let `r0_ah` be the zero of the untruncated `LJ_ah(r) + K_ah·pair_erfc(αr)/r` below its minimum (bisection on the host), clamped to `r_lj` (the zero can lie beyond `r_lj`, past which the true `u_ah` no longer includes the Lennard-Jones term — a pose there can be strongly attractive, not `≥ 0` as the unclamped envelope would assert), then `u_ah ≥ 0` on `(0, r0]` and `u_ah ≥ −ε_ah − |K_ah|·erfc(α r0)/r0` on `[r0, r_c]`. `B_s` is the sum of those magnitudes over all sites and all host atoms of system `s`, plus the reciprocal bound `R_s = KE Σ_k kprefactor_k · 2|S_host,k| · Σ_a|q_a|`. `B_s` is computed at batch construction, stored per system (`hardcore_bound` in `src/reject.jl`), and is `Inf` (no rejection in that system) if any pair has `ε_ah = 0` with `K_ah < 0` (tested). `hardcore_bound` memoizes `find_r0` per `(σ, ε, K)` triple within one call (many atoms share a compact type and charge); `FrameworkBatch` computes every framework's bound in a `Threads.@threads` pass after the rest of construction, one memo `Dict` per thread.
- ☑ **Core radii.** Per `widom` call (they depend on `T`), per system, per (guest site `a`, host LJ type `t`): `ρ_at` is the first root, scanning up from `r → 0`, of `LJ_at(r) − |K_min(a,t)|/r = (θ + 2)·kT + safety + B_s − c_s` (`find_rho2`/`build_rejection_tables`, `θ = theta_F(Float64)` regardless of `F` — see "Rejection rule" above), clamped to `r_lj` for the same reason as `r0_ah` above, where `K_min(a,t)` is the most negative `K_ah` over the system's atoms of type `t` (zero if none is negative, `kmin_table`). Every `r < ρ_at` satisfies the rejection condition. Stored as `ρ²`, in a table indexed by a batch-local compact type index. The safety term is `safety = 2·n·eps(F)·(B_s + |c_s|) + 4e-6·B_s` (`F` `FrameworkBatch`'s own float type, since `insertion_energy` sums on the device in `F`), with `n = N_sites·natoms_s + nk_s + 8`: `2·n·eps(F)·(B_s+|c_s|)` bounds recursive-summation rounding error for `n` terms of magnitude at most `B_s + |c_s|` (the standard `(n-1)·u·Σ|x_i|` bound with unit roundoff `u = eps(F)/2`); `4e-6·B_s` covers `pair_erfc_dev`'s own relative error (measured up to 1.51e-6 in Float32 over `[0, PAIR_ERFC_XMAX]`), applied once per Coulomb term. `find_rho2` and `find_r0` bisect to a relative interval width of `4·eps(T)` (not a fixed iteration count), with an iteration cap that throws rather than looping past it; `find_rho2` requires `margin > 0` (throws by name otherwise) and returns `ρ² = 0` directly for a zero `epsilon_at`, both checked before any loop. `build_rejection_tables` runs its per-system loop under `Threads.@threads :static`, one memo `Dict` per thread keyed on `(σ, ε, kmin_at, margin)` (a batch of identical tiled systems shares the same margin and per-type values, so only the first system per thread pays a fresh bisection).
- ☑ **Compact types.** `FrameworkBatch` remaps LJ types to the types actually present in the batch (framework atoms and guest sites); `types`, `sigma`, `epsilon` use the compact index, and `compact_to_orig` is the public mapping back to the force field's own index. The guest's own `guest_types_orig`/`guest_sites_orig`/`guest_charges_orig` are also kept, and `widom` throws a clear error if a later call passes a guest that does not match all three (not just types — every precomputed quantity depends on sites and charges too), naming both the passed guest's and the batch's own types/sites/charges.
- ☑ **Phase 0 kernel.** One work-item per insertion (`hardcore_kernel!`). Each guest site computes its own home cell from its actual position (wrapped into `[0,1)` first via `wrap_frac`, since a site's position can lie outside the stored cell) and scans the stencil around it (reach from the call's largest `ρ_at`, computed per system per call), with one minimum image per atom; the flag is set if any `r² < ρ²_at`. A single shared stencil centered on the pose's reference point instead under-rejects when a site's own position is far enough from that reference point (a large `r_guest`, or a cell width narrower than the reach) that a host atom within the site's own radius falls outside that shared stencil — verified against a per-site brute-force scan (12 and 574 such misses over 400,000 poses on the shared-stencil form, zero after the per-site fix, both directions). Output: one `UInt8` per insertion. Same GPU-safety rules as the energy kernel; compiles and runs on this machine's RTX 3050 (`:gpu` test item, CUDA), which now also checks phase 0 directly (bit-exact against the CPU backend) rather than only `widom`'s end-to-end aggregate.
- ☑ **Compaction and phase 1.** The host reads the chunk's flags, builds the survivor index list in order, uploads it, and launches the energy kernel (`widom_kernel!`) over `nsurvivors` work-items, each reading its pose through the index and writing `ΔU` back at its original position. Rejected insertions add to the sample counts with zero weight (`boltzmann_weight`). A chunk with no survivors launches phase 1 not at all. The host pass was not replaced by a device-side scan: not measured to matter (phase 0 is 0.9–3.0% of the kernel-path time at the chosen width, see below).
...
- ☑ **Gate before building kernels:** measured on the CPU (JuliaMCP session), for CO2 in RUBTAK 3×3×3 at 298.15 K, 40,000 poses, commit 6ef0c7d: host-rule rejection fraction 40.81% (Float64) / 40.82% (Float32), next to the fraction of exact zeros 68.58% / 78.19% (matching the design's recorded 68%/78% baseline). `B_s` = 127,118 kT in both precisions (order 10⁵ kT as expected). Both rejection fractions are above the 25% stop threshold and close to the ~40% estimate — continued to the kernels.
- ☑ **Tests.** `θ_F` by construction (`test/reject_tests.jl`); `B_s` against a brute-force evaluation of the bounded quantity on 2,000 random poses (the bound holds for every sampled pose: `ΔU − u_trigger ≥ −B_s + c_s`); no rejected pose has `W > 0` when its full energy is computed, 10⁵ poses per precision on the actual `find_rho2` radii (host math), plus a dedicated reproducer where the Lennard-Jones cutoff is shorter than the core radii (a single charged host atom, cutoff far below the unclamped radii) checked the same way and against two-phase/single-phase equality over 500,000 poses; the production-kernel check is the `:gpu` item plus the two-phase/single-phase exact-equality test, both of which ran with real rejections and no discrepancy; the phase-0 kernel's flags equal a per-site brute-force scan over every atom (no cell list), in both directions, over 400,000 poses total (triclinic RUBTAK 3×3×3 + CO2, and a small cubic cell whose guest's `r_guest` exceeds the cell width); two-phase `widom` results equal single-phase results (`widom_singlephase`, non-exported) exactly (`==`) for fixed seeds on the CPU backend, for one framework, several frameworks, a neutral guest, a framework where `B_s = Inf`, and the Lennard-Jones-cutoff reproducer above (`test/widom_tests.jl`, `test/reject_tests.jl`; small chunk sizes relative to the ~41% rejection fraction make both a zero-survivor and an all-survivor chunk overwhelmingly likely across the run, rather than being individually forced); `build_rejection_tables`'s and `FrameworkBatch`'s threaded passes match a serial recomputation exactly, and the wider suite passes under an 8-thread run in addition to the default; GPU results bit-equal to CPU on this machine (RTX 3050, `:gpu` item, `rtol = 1e-10` for `widom`'s aggregate, exact for phase 0's own flags); kUPS cross-code `:slow` item unchanged (passed); `bench/audit.jl` covers `insertion_energy`'s new (linear) signature and phase 0's own callees (`home_cell_dev`, `wrap_frac`, `stencil_start_count`, `wrap_cell`, `cell_linear`) not already covered by `insertion_energy`, both precisions, full mode (28 findings, 0 failing).
- ☑ Measured and recorded (RTX 3050, `bench/gpu`, RUBTAK 3×3×3 + CO2, commit 6ef0c7d): rejection fraction (kernel path) 40.5% both precisions; phase 0 / phase 1 / total kernel-path time for a 65,536-insertion chunk, cellwidth 2 Å: Float64 7.26 ms / 838.5 ms / 845.7 ms (against 3.7 / 836.9 / 840.6 ms at 3653c6f); Float32 1.18 ms / 38.4 ms / 39.5 ms (against 0.68 / 38.7 / 39.4 ms at 3653c6f); end-to-end `widom` time for 10⁶ insertions, one framework: Float64 13.04 s (76,715 ins/s, against 13.02 s at 3653c6f); Float32 0.672 s (1.489M ins/s, against 0.674 s at 3653c6f). Phase 0 costs more per site now that each site scans its own stencil (about 2× at Float64, 1.7× at Float32), but stays a small fraction of the kernel path, so the total and end-to-end times are unchanged within measurement noise. Speedup against the E2b baseline (commit 082c656: whole kernel 63.4 ms Float32, 1245.9 ms Float64): Float32 1.60×, Float64 1.47×. R9700 (galen, ROCm, commit a4c86a7): rejection fraction (kernel path) 40.5% both precisions; phase 0 / phase 1 / total kernel-path time for a 65,536-insertion chunk, cellwidth 2 Å: Float64 1.45 ms / 232.41 ms / 233.86 ms; Float32 0.35 ms / 18.49 ms / 18.84 ms; end-to-end `widom` time for 10⁶ insertions, one framework: Float64 3.748 s (266,793 ins/s); Float32 0.4037 s (2,476,918 ins/s).

  Cellwidth sweep for phase 0 (RTX 3050, 65,536-insertion chunk, `bench/gpu/cellwidth_sweep.jl`, mirroring `bench/widom_bench.jl`'s kernel-only path; measured before the per-site stencil change, at commit da327a6):
  ...
  a stencil that spans less than the whole grid the way E2's wider, cutoff-sized cells did.

  Host cost of the rejection tables (neuromancer, CPU backend, RUBTAK 3×3×3 + CO2): `build_rejection_tables` at 1 / 1,024 / 32,768 tiled systems, 11.3 µs / 10.5 ms / 334 ms before memoization and threading, against 40.9 µs / 114 µs / 3.28 ms after (`bench/widom_scaling.jl`'s `rejection_tables_s`, commit 1ea8993); `FrameworkBatch` construction for one RUBTAK 3×3×3 framework, 175.4 ms before against 163.4 ms after (`bench/widom_bench.jl`'s `batch_construction_s`).
